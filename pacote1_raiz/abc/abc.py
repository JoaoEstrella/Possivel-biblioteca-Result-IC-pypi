...
class Abc:
    ...
    def start(self, pop):
        ...
        pop.pBest['fitness'] = np.empty_like(pop.pBest['value'])
        pop.pBest['fitness'][0] = pTemp['fitness'][0]
    ...
    def nextGen(self, pop):
        ...
        for emBee in employedBee:  # Abelha i
            ...
            j = np.random.choice(pop.ranges.shape[0])  # Variavel j

            # print('d:', j)

            neighbour = np.random.choice(employedBee)  # Abelha k
            while emBee is neighbour:
                neighbour = np.random.choice(employedBee)
        ...
        for k in range(len(employedBee)):
            ...
            onBee = np.random.choice(employedBee, None, p=prob)
            # print(onBee)
